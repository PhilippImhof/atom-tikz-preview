...
getTikzPreamble(pathOrEditor) {
  ...
  getTikzBlockAtCursor(cursor, editor) {
    ...
    do {
      start--;
    } while (editor.scopeDescriptorForBufferPosition([start, 0]).getScopesArray().includes(TIKZ_SCOPE) && start>0)

    // move end down until the end of the tikz scope
    do {
      end++;
    } while (editor.scopeDescriptorForBufferPosition([end, 0]).getScopesArray().includes(TIKZ_SCOPE) && end<lineCount)
